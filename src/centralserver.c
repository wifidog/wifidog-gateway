                    ...
                    const char *token, unsigned long long int incoming, unsigned long long int outgoing, unsigned long long int incoming_delta, unsigned long long int outgoing_delta)
{
    ...
    if(config -> deltatraffic) {
    	   snprintf(buf, (sizeof(buf) - 1),
             "GET %s%sStage=%s&ClientIP=%s&ClientMAC=%s&Token=%s&Incoming=%llu&Outgoing=%llu&DeltaIncoming=%llu&DeltaOutgoing=%llu&GatewayID=%s HTTP/1.0\r\n"
    		...
    		snprintf(buf, (sizeof(buf) - 1),
             "GET %s%sStage=%s&ClientIP=%s&ClientMAC=%s&Token=%s&Incoming=%llu&Outgoing=%llu&GatewayID=%s HTTP/1.0\r\n"
             "User-Agent: WiFiDog %s\r\n"
             "Host: %s\r\n"
             "\r\n",
             auth_server->authserv_path,
